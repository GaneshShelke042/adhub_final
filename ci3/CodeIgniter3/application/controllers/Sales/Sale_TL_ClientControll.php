...
class HR_ClientControll extends CI_Controller
{
 ...
 // ^^^^^^^^^^^^^^^^^^^^^^^ This is for Permission  ^^^^^^^^^^^^^^^^^^^^^^
    ...
    function clientView()
    {
        ...
        $this->load->view('Sales\Client\ViewClientD.php', $data1);
    ...
    function addNew_Cr_Client()
    {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('Sales\Client\AddnewClient.php',$data);
        } else {
            ...
            // ^^^^^^^^^^^^^^^^^^^^^^^ THis is  for GenrateCalender  ^^^^^^^^^^^^^^^^^^^^^^
                ...
                redirect(base_url() . 'index.php/Sales/Sale_TL_ClientControll/clientView');
    ...
    function editData($userData)
    {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('Sales\Client\EditAddnewClientData', $data);
        } else {
