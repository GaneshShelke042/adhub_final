...
class HR_ClientControll extends CI_Controller
{
 ...
 // ^^^^^^^^^^^^^^^^^^^^^^^ This is for Permission  ^^^^^^^^^^^^^^^^^^^^^^
    ...
    function clientView()
    {
        ...
        $this->load->view('HR\Client\ViewClientD.php', $data1);
    ...
    function addNew_Cr_Client()
    {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('HR\Client\AddnewClient.php',$data);
        } else {
            ...
            // ^^^^^^^^^^^^^^^^^^^^^^^ THis is  for GenrateCalender  ^^^^^^^^^^^^^^^^^^^^^^
                ...
                redirect(base_url() . '/index.php/HR/HR_ClientControll/clientView');
    ...
    function editData($userData)
    {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('HR\Client\EditAddnewClientData', $data);
        } else {
